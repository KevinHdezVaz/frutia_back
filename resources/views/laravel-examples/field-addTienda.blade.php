            ...
            <form method="POST" action="{{ route('product.store') }}" enctype="multipart/form-data">
                ...
                <div class="form-group">
                    ...
                    <input type="text" id="colors" name="colors" 
                        ...
                        value="{{ old('colors') }}" placeholder="Rojo, Azul, Negro">
                    @error('colors')
                        <div class="invalid-feedback">{{ $message }}</div>
                    @enderror
                ...
                <div class="form-group">
                    ...
                    <input type="text" id="sizes" name="sizes" 
                        ...
                        value="{{ old('sizes') }}" placeholder="S, M, L, XL">
                    @error('sizes')
                        <div class="invalid-feedback">{{ $message }}</div>
                    @enderror
                ...
                <div class="form-group">
                    ...
                    <input type="number" id="units" name="units" 
                        ...
                        value="{{ old('units', 1) }}" min="1" step="1">
                    @error('units')
                        <div class="invalid-feedback">{{ $message }}</div>
                    @enderror
                ...
                <div class="d-flex justify-content-end mt-4">
                    <a href="{{ url('product') }}" class="btn btn-light m-0">Cancelar</a>
                    <button type="submit" class="btn bg-gradient-primary m-0 ms-2">Agregar Producto</button>
                </div>
            </form>
        </div>
    </div>
